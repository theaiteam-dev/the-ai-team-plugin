...
**Stockwell does NOT issue `--return-to` rejections.** His Final Mission Review runs after every item is in terminal `done` — nobody runs `--return-to` against a `done` item (see `adr/0005-done-is-terminal-no-in-mission-rework.md`). A Stockwell rejection is report-only: he names the items and issues, reopening them is a manual operator action, and the tail restarts at Frankie once the named items are back in `done`.
...
6. **Frankie** walks the mission's full Definition of Done against the running app and produces an evidence bundle ← MANDATORY, RUNS BEFORE Stockwell (skipped only on repos whose execution contract declares no drivable surface — see `scripts/hooks/lib/qa-contract.js`). A failure halts here and surfaces to the operator (no automated bounce — `done` is terminal).
