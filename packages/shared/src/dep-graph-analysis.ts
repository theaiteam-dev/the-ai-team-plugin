 ...
 */
export function computeDepGraphMaxPerStage(items: DepGraphItem[]): number {
  ...
  while (unresolved.size > 0) {
    ...
    for (const id of unresolved) {
      ...
      if (!item.dependencies.every((dep) => waveOf.has(dep))) continue;
    ...
    if (!progress) break; // guard against cycles or missing dep references
  }
