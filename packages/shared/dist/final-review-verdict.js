 ...
 */
/**
 * The exact trailer, after the tolerated decorations below have been peeled
 * off. Case-sensitive and uppercase on purpose: this is a machine-read
 * contract line that agents/stockwell.md and the playbooks spell out
 * verbatim, and a near-miss must fail closed (block + re-POST) rather than be
 * guessed at.
 */
const VERDICT_TRAILER_RE = /^VERDICT:[ \t]*FINAL[ \t]+(APPROVED|REJECTED)$/;
/** A whole line wrapped in markdown bold: `**...**`. */
...
export function parseFinalReviewVerdict(review) {
    ...
    if (last === null)
        return 'unknown';
    const bold = BOLD_WRAPPED_RE.exec(last);
    const core = bold ? bold[1].trim() : last;
    const match = VERDICT_TRAILER_RE.exec(core);
    if (!match)
        return 'unknown';
    return match[1] === 'APPROVED' ? 'approved' : 'rejected';
}
