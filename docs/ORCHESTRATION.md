...
Pipeline agents communicate directly with each other (peer-to-peer handoffs). Hannibal dispatches only the first agent per item and receives FYI/ALERT messages — it does NOT route every handoff.
...
1. Agent calls `ateam agents-stop agentStop --advance` → API advances item to next stage
2. Agent sends START message directly to the next agent (e.g., Murdock → B.A.)
3. Next agent ACKs within 20 seconds
4. Agent sends FYI to Hannibal ("handoff confirmed")
5. If no ACK within 20s: agent sends ALERT to Hannibal ("handoff timed out") — Hannibal dispatches next agent manually
...
Native teams handles orchestration; ateam CLI handles persistence and stage transitions.
