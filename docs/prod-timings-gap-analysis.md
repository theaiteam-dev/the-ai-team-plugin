...
P2P handoffs dramatically reduced dispatch latency. B.A.→Lynch is now near-instant (0.31 min avg). Max values remain spiky (Murdock→B.A. hits ~4.7 min worst case across all missions), likely from first-of-wave dispatches or WIP stalls.
...
4. **Lynch is the fastest worker** (avg 0.9 min), suggesting reviews are lightweight relative to implementation and testing.
