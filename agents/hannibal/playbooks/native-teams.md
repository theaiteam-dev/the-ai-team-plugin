...
At the very start of the mission (before team creation), determine the instance count:

```
# 1. Check for manual concurrency override (env var or CLI flag)
concurrency_override = $ATEAM_CONCURRENCY  # or --concurrency flag value, or null

# 2. Compute adaptive scaling
if concurrency_override is set and > 0:
    N = concurrency_override
else:
    # Use adaptive scaling: min(dep_graph_max, memory_ceiling)
    dep_graph_result  = Bash("ateam deps-check checkDeps --json")
    dep_graph_max     = dep_graph_result.maxParallelPerStage  # or default to 1
    memory_ceiling    = floor(available_memory_mb / 512)      # ~512 MB per agent instance
    N = max(1, min(dep_graph_max, memory_ceiling))

# 3. Persist the scaling rationale via mission API
scaling_rationale = {
    instanceCount: N,
    depGraphMaxPerStage: dep_graph_max,
    memoryBudgetCeiling: memory_ceiling,
    bindingConstraint: (concurrency_override ? "override" : (dep_graph_max <= memory_ceiling ? "dep_graph" : "memory")),
    concurrencyOverride: concurrency_override or null
}
# Store via: PATCH /api/missions/{missionId} body: {scalingRationale: scaling_rationale}
# or via ateam CLI if available: ateam missions-update updateMission --id {missionId} --scalingRationale '{json}'

# 4. If N == 1 → use single-instance mode (names: murdock, ba, lynch, amy — no suffix)
