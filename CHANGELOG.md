...
- **Tick-based mission controller (deterministic Go planner):** The A(i)-Team now features a deterministic mission controller replacing in-prompt orchestration logic. The `ateam controller tick` command reads current mission state from the API (`/api/missions/current`, `/api/items`, `/api/controller-checkpoint`, `/api/missions/current/health-report`) plus the local pool filesystem, computes a small JSON action plan considering WIP limits and dependency readiness, and emits actions (dispatch, message, final-review, recovery) that Claude executes. Ticks are idempotent — each action carries a deterministic ID (`missionId:itemId:kind:agent:sequence`) so re-running before checkpoint confirmation produces identical plans. Replaces 1000+ lines of orchestration prompt with stateless Go computation. (#004-006)
- **Controller checkpoint CLI (get/put/confirm):** Operators atomically persist completed action IDs via `ateam controller checkpoint get/put/confirm` subcommands. The checkpoint tracks which actions have been executed, enabling deterministic recovery after stalls. Stored in the Prisma schema (`MissionControllerCheckpoint` table) with `GET`/`PUT`/`PATCH /api/controller-checkpoint/{missionId}` endpoints. (#005)
- **Tick recovery actions (release/move/stale-claim detection):** When a pool slot is claimed but idle for >600 seconds, tick emits a `release` action. When items are WIP-blocked but dependencies resolve, tick emits a `move` action. Ambiguous stalls (e.g., "agent is busy but no action id was just emitted") route through `needsJudgment` instead of guessing. (#006)
- **`/ai-team:tick` slash command:** New command executes one full controller tick: reads the action plan via `ateam controller tick`, performs each action via Claude Code primitives (dispatch, SendMessage, TaskCreate, etc.), confirms completion in the checkpoint, and schedules the next tick. Replaces polling-based orchestration with deterministic event-driven workflow. (#007)
- **Wire `/ai-team:run` and `/ai-team:resume` to tick loop:** Running `/ai-team:run` or `/ai-team:resume` now hands control to the tick loop instead of loading the orchestration playbook. In steady state, Hannibal arms a single heartbeat ScheduleWakeup → `/ai-team:tick`, eliminating 25-minute prompt-cache churn from playbook reloads. (#008)
- **Delegate `/ai-team:healthcheck` to controller tick:** The health-check slash command now invokes `ateam controller tick --dry-run` to inspect the current mission snapshot (board state, pool status, in-flight-item activity). Deterministic inspection replaces ad-hoc polling. (#009)
