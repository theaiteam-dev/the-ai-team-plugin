...
- **Accurate per-message token attribution:** End-to-end tracking of token usage at the individual message level, replacing the prior per-transcript summation. New `MessageTokenUsage` Prisma model stores baseAgentName, model, and token counts (inputTokens, outputTokens, cacheCreationTokens, cacheReadTokens) keyed by (missionId, agentName, message index). Observer hooks (`observe-stop.js`, `observe-subagent.js`) now parse transcripts and emit a fire-and-forget `POST /api/hooks/token-usage` for each message with full validation including agentName — with a scalar bridge for legacy HookEvent telemetry. Mission token-usage aggregation (`GET /api/missions/{id}/token-usage`) rewrites to SUM from MessageTokenUsage grouped by (baseAgentName, model), eliminating the prior HookEvent path and its deduplication logic. Includes new `/api/hooks/token-usage` endpoint with comprehensive validation. Token cost calculation and UI display remain unchanged; the foundation is now per-message granular.
