...
- **Pipeline parallelism with multi-instance agents:** Runnable pool architecture enabling multiple concurrent instances of the same agent (e.g., `murdock-1`, `murdock-2`, `ba-1`, `ba-2`). Native teams orchestration dispatches work items to available pool members, allowing true parallel feature development within memory and dependency constraints. See `agents/hannibal/playbooks/native-teams.md` for pool-based handoff protocol (#037-047).
