...
- New CLI verbs. The existing `ateam agents-start agentStart` / `ateam agents-stop agentStop` commands stay; the only behavior change is that `--itemId` becomes optional for the five mission agents.
...
7. The system shall accept `--outcome rejected` on phase-stop *only for `stockwell`*. A Stockwell rejection shall write to BOTH (a) the matching `MissionPhase` row (status, summary, and the list of rejected item IDs) AND (b) each named item — incrementing the item's `rejection_count` and recording the rejection in its `work_log`. Items that hit the rejection cap shall transition to `blocked` per the existing item rejection policy. All other phase agents may only complete or fail.
...
12. Each successful `phase-start` shall transition `Mission.state` to the matching `*_in_progress` value if and only if the transition is legal. Each successful `phase-stop` shall transition out of that state into the next legal state (or `failed` / `archived` on failure / archive).
...
4. The API surface change shall be backwards-compatible for all existing `ateam` CLI versions: older CLIs sending `--itemId` for phase agents will receive a validation error message that names the new pattern.
