...
| Kind | How to execute |
|------|----------------|
| `dispatch` | Fetch the item prompt with `ateam items renderItem --id <itemId>`. **Legacy mode**: spawn the named agent via `Task(subagent_type, prompt)`. **Native-teams mode**: spawn via `TeamCreate`. Do NOT inline or invent prompts — always use `renderItem`. |
...
| `release` | `ateam board-release releaseItem --itemId <id>` **and** `ateam pool release --agent <name>` as named by the action |
...
| `ateam pool release --agent <name>` | Release a stale pool slot |
