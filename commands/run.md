...
5. `probing → staged`: Amy verifies (VERIFIED); on a FLAG she returns the item to `testing` or `implementing` by the earliest-flagged-stage rule (test gap — alone or alongside an impl bug — goes to `testing`; an impl bug alone goes to `implementing`). `probing → ready` exists in the transition matrix for manual operator recovery only — no pipeline agent uses it as a rejection target. `staged` is the per-item pipeline's real terminal stage (WI-786/787), a holding pen awaiting mission-tail verification
